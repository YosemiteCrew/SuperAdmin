...
import { FaPlus, FaCamera, FaTrash, FaMinus, FaArrowLeft, FaCopy } from "react-icons/fa";
import { useRouter } from 'next/navigation';
import axios from 'axios';
import Image from 'next/image';
import PublishLaterModal from '@/app/Components/PublishLaterModal/PublishLaterModal';
...
const DuplicateAssessmentPage: React.FC<DuplicateAssessmentPageProps> = ({ assessmentId }) => {
  ...
  const [isSchedule, setIsSchedule] = useState({
    type: 'none' as 'scheduled' | 'work-in-progress' | 'pending' | 'none',
  ...
  const [publishLaterLoading, setPublishLaterLoading] = useState(false);
  ...
  const fetchAssessment = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setAssessmentName(`${assessment.name} (Copy)`);
        setAssessmentType(assessment.type);
        setCategory(assessment.category);
        setDescription(assessment.description);
        
        // Preserve the original assessment's publish status and schedule
        setIsPublished(assessment.isPublished || false);
        setIsSchedule({
          type: assessment.isSchedule?.type || 'none',
          date: assessment.isSchedule?.date ? new Date(assessment.isSchedule.date) : null
        });
  ...
  const handlePublishNow = async () => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/assessment/create`,
        {
          ...formData,
          isPublished: true,
          isSchedule: {
  ...
  const handleSchedulePublish = async (scheduledDate: Date) => {
    try {
      setPublishLaterLoading(true);
      ...
      if (response.data.success) {
        alert('Assessment scheduled for publishing!');
  ...
  return (
    <AdminDashboardLayout>
        ...
        <Container>
          <div className="py-4">
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
              ...
              <h2 style={{ fontWeight: '600', color: '#333', margin: 0 }}>Duplicate Assessment</h2>
            </div>
      ...
      {/* PublishLaterModal */}
      <PublishLaterModal
        show={showPublishLaterModal}
        onHide={() => setShowPublishLaterModal(false)}
        onSchedule={handleSchedulePublish}
        loading={publishLaterLoading}
      />
    </AdminDashboardLayout>
  );
};

export default DuplicateAssessmentPage; 