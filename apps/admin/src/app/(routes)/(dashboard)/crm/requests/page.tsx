...
function RequestCard({
  ...
  return (
    <article className={`${CARD} flex flex-col gap-2 px-5 pb-[14px] pt-4`}>
        ...
        <div className="flex min-w-0 items-center gap-[11px]">
          ...
          <div className="flex min-w-0 flex-col">
            ...
            <p className="truncate text-[12px] text-[color:var(--ink-faint)]">
              {r.email}
              {r.company ? ` · ${r.company}` : ''}
              {/* Added to the intake after this card was designed; a lead's phone
                  number is on the row in `main` and must not disappear here. */}
              {r.phone ? ` · ${r.phone}` : ''}
            </p>
          </div>
        </div>
      ...
      <DataRequestFlag email={r.email} subject={r.subject} message={r.message} />
