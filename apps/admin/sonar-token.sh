...
# with service name "sonar-token-superadmin"), then a gitignored .sonar-token
# file beside this script. The file is the legacy path and is only still read so
# an unmigrated checkout keeps working; prefer the Keychain.
#
...
# The service name is defined once and interpolated everywhere else — the read
# below and every "here is how to create the item" hint — so no two of them can
# end up naming different services.
#
# It carries this app's name rather than being left unqualified on purpose. The
# documented write is `add-generic-password -U`, which overwrites a matching item
# in place, and the lookup below passes no account, so an unqualified name is a
# slot any other tool on the machine may already be using: whichever was set up
# last would silently own it and the other would read the wrong token.
SONAR_KEYCHAIN_SERVICE="sonar-token-superadmin"
...
_sonar_token_from_keychain() {
  ...
  _kc_out="$(mktemp)"
  ( security find-generic-password -s "$SONAR_KEYCHAIN_SERVICE" -w >"$_kc_out" 2>/dev/null ) &
  _kc_pid=$!
  _kc_i=0
