...
export type PetParentRow = {
  name: string;
  email: string;
  city: string;
  country: string;
  practiceAssociation: string;
  pets: number;
  appointments: number;
  lastActivity: string;
  status: string;
...
type PetParentsOverviewProps = {
  rows: PetParentRow[];
  totalCount: number;
};

export function PetParentsOverview({ rows, totalCount }: PetParentsOverviewProps) {
    ...
    <section className="relative">
      <div className="sticky top-0 z-10 -mx-1 -mt-1 border-b border-transparent bg-white/95 px-1 pb-4 pt-1 backdrop-blur-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <h3 className="text-xl font-semibold text-[#302F2E]">Pet Parents Overview</h3>
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              <input
                type="search"
                placeholder="Search pet parent"
                value={search}
          ...
          <table className="w-full min-w-[900px]">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/50">
                ...
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-600">Practice Association</th>
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-600">Pets</th>
              ...
              {rows.map((row, i) => (
                <tr key={`${row.name}-${i}`} className="border-b border-gray-100 last:border-0">
                  <td className="px-6 py-4">
                    <div>
                      <span className="block text-sm font-medium text-[#302F2E]">{row.name}</span>
                  ...
                  <td className="px-6 py-4 text-sm text-gray-600">{row.practiceAssociation}</td>
                  <td className="px-6 py-4 text-sm font-medium text-[#302F2E]">{row.pets}</td>
