...
function run(body, expected = SHA, required) {
  const dir = mkdtempSync(path.join(tmpdir(), 'intake-readiness-'));
  const file = path.join(dir, 'health.json');
  writeFileSync(file, typeof body === 'string' ? body : JSON.stringify(body));
  const args = [SCRIPT, file, expected];
  if (required !== undefined) args.push('--require', required);
...
test('an empty-string buildSha is exit 2, distinct from a real sha', () => {
  const { status, stderr } = run({
    buildSha: '',
    intake: { contact: 'configured', consent: 'configured' },
  });
  assert.equal(status, 2);
  assert.match(stderr, /buildSha is absent or empty/);
});
...
test('a live sha that is not the expected one is exit 0 but says NOT_DEPLOYED', () => {
  const { status, stdout } = run({
    buildSha: OLDER,
    intake: { contact: 'unconfigured', consent: 'unconfigured' },
  });
...
test('the expected sha live with contact unconfigured is exit 1 and names the field', () => {
  const { status, stdout, stderr } = run({
    buildSha: SHA,
    intake: { contact: 'unconfigured', consent: 'configured' },
  });
...
test('without --require, the expected sha live with consent unconfigured names only consent', () => {
  const { status, stdout } = run({
    buildSha: SHA,
    intake: { contact: 'configured', consent: 'unconfigured' },
  });
  assert.equal(status, 1);
...
test('consent may be unconfigured when only contact is required', () => {
  const { status, stdout } = run(
    { buildSha: SHA, intake: { contact: 'configured', consent: 'unconfigured' } },
    SHA,
    'contact'
  );
  assert.equal(status, 0);
...
test('contact being unconfigured fails when contact is required', () => {
  const { status, stdout } = run(
    { buildSha: SHA, intake: { contact: 'unconfigured', consent: 'configured' } },
    SHA,
    'contact'
  );
  assert.equal(status, 1);
...
test('an unknown required intake is exit 2', () => {
  const { status, stderr } = run(
    { buildSha: SHA, intake: { contact: 'configured', consent: 'configured' } },
    SHA,
    'bogus'
  );
  assert.equal(status, 2);
  assert.match(stderr, /unknown intake name: bogus/);
});

test('a comma-separated requirement checks both named intakes', () => {
  const { status, stdout } = run(
    { buildSha: SHA, intake: { contact: 'unconfigured', consent: 'unconfigured' } },
    SHA,
    'contact,consent'
  );
  assert.equal(status, 1);
...
test('intake values other than configured are treated as not configured', () => {
  const { status, stdout } = run({
    buildSha: SHA,
    intake: { contact: 'unknown', consent: 'configured' },
  });
  assert.equal(status, 1);
  assert.match(stdout, /contact/);
});
